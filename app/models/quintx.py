...
class CaseReport(db.Model):
    ...
    additional_matches = db.relationship('AdditionalMatch', back_populates='case_report', cascade='all, delete-orphan')
...
class AdditionalMatch(db.Model):
    __tablename__ = 'additional_matches'

    id = db.Column(db.Integer, primary_key=True)
    case_report_id = db.Column(db.Integer, db.ForeignKey('case_reports.id'), nullable=False)

    bacteria_name = db.Column(db.String(100))
    ncbi_id = db.Column(db.String(100))
    tax_id = db.Column(db.String(100))
    match_score = db.Column(db.Float)

    phage_matches = db.relationship('AdditionalPhageMatch', back_populates='additional_match', cascade='all, delete-orphan')
    case_report = db.relationship('CaseReport', back_populates='additional_matches')


class AdditionalPhageMatch(db.Model):
    __tablename__ = 'additional_phage_matches'

    id = db.Column(db.Integer, primary_key=True)
    additional_match_id = db.Column(db.Integer, db.ForeignKey('additional_matches.id'))
    phage_id = db.Column(db.String(150), db.ForeignKey('phages.phage_id'))

    additional_match = db.relationship('AdditionalMatch', back_populates='phage_matches')
    phage = db.relationship('Phages')
